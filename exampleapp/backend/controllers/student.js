...
async function findByMatricula(req, res) {
    const exist = await Student.find({matricula: req.params.matricula});

    if (exist) {
        res.json({ student: exist });
    } else {
        res.status(400).json({ msg: `No student with the matricula ${req.params.matricula}`});
    }
};
...
async function deleteByMatricula(req, res) {
    const exist = await Student.find({matricula: req.params.matricula});
    console.log(exist);
    if (exist.length > 0) {
        await Student.deleteMany({matricula: req.params.matricula});
        res.json( { msg: 'Student deleted', students: await Student.find() });
    } else {
        res.status(400).json({ msg: `No student with matricula ${req.params.matricula}`});
    }
};
...
module.exports = {findAll, findById, findByMatricula, findByEmail, save, update, deleteById, deleteByMatricula, deleteByEmail};
